...
module.exports = {
  ...
  checkPolicy : function(dependencies, policy) {
    ...
    _.each(dependencies, function(val, key) {
      ...
      //Exempt URL dependencies, since they must be for concrete tags and will not accept modifiers
      if (val.indexOf('http') >= 0 || val.indexOf('git') >= 0) {
        return;
      }

      var justTheVersion = val.match(/[~\^\*x0-9]+\.([~\^\*x0-9]+\.)?.+/)[0];

      var depFrags = justTheVersion.split(".");
